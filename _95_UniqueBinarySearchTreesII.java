...
public class _95_UniqueBinarySearchTreesII {
     ...
     * @return
     */
    //关于这道题有个疑问 就是输出难道不应该是List<List<TreeNode>>吗？
    ...
    public List<TreeNode> genTreeList(int start, int end) {
        ...
        if (start > end) {//这是只需将null添加到list中 并不需要返回上一层
            list.add(null);
        }
        for (int idx = start; idx <= end; idx++) {//这段代码属实难以理解
            ...
            for (TreeNode left : leftList) {
                for (TreeNode right : rightList) {
                    ...
                    root.left = left;
                    root.right = right;
                    list.add(root);//list直接添加TreeNode
                }
            }
        }
        return list;//在递归函数最后return
    }
}
