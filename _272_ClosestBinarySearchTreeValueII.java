...
public class _272_ClosestBinarySearchTreeValueII {
    ...
    public List<Integer> closestKValues(TreeNode root, double target, int k) {
        LinkedList<Integer> res = new LinkedList<>();//knows to use LinkedList is the very key of it
    ...
    private void helper(LinkedList<Integer> res, TreeNode root, double target, int k) {
        ...
        if (res.size() == k) { //if we remove this if,then whole code becoming in order tranversal
            if (Math.abs(target - root.val) < Math.abs(target - res.peekFirst())) { //if current root.val is closer to target than the head of linkedlist do
                res.removeFirst();//then we remove the head of linkedlist
            } else return; // or else, we do nothing but return
        }
        res.add(root.val);
